.. Alexa Web Information Service: UrlInfo Action

.. toctree::
    :maxdepth: 1

####################################################
Alexa Web Information Service (AWIS): UrlInfo Action
####################################################

A typical JSON-ified AWIS records presents as follows::

  {
    "UrlInfoResult":{
      "Alexa":{
        "ContentData":{
          "DataUrl":{
            "@type":"canonical",
            "$":"feedblitz.com"
          },
          "SiteData":{
            "Title":{
              "$":"FeedBlitz: Add Email to your Blog and RSS Feeds"
            },
            "Description":{
              "$":"FeedBlitz is a service that monitors blogs, RSS feeds and Web URLs to provide greater reach for feed publishers. FeedBlitz takes all the headache out of converting feed and blog updates into email digests, delivered daily to subscribers inboxes. FeedBlitz manages subscriptions, circulation tracking, testing, and is compatible with all major blogging platforms and services such as Blogger, Typepad and FeedBurner."
            },
            "OnlineSince":{
              "$":"16-May-2005"
            }
          },
          "Speed":{
            "MedianLoadTime":{
              "$":1969
            },
            "Percentile":{
              "$":43
            }
          },
          "AdultContent":{
            "$":"no"
          },
          "Language":{
            "Locale":{
              "$":"en"
            },
            "Encoding":{
              "$":"us-ascii"
            }
          },
          "LinksInCount":{
            "$":15943
          },
          "OwnedDomains":{
            "OwnedDomain":{
              "Domain":{
                "$":"feedadvisor.com"
              },
              "Title":{
                "$":"feedadvisor.com"
              }
            }
          }
        },
        "Related":{
          "DataUrl":{
            "@type":"canonical",
            "$":"feedblitz.com"
          },
          "RelatedLinks":{
            "RelatedLink":[
              {
                "DataUrl":{
                  "@type":"canonical",
                  "$":"www.zookoda.com/"
                },
                "NavigableUrl":{
                  "$":"http://www.zookoda.com/"
                },
                "Title":{
                  "$":"Blog Promotion"
                }
              },
              {
                "DataUrl":{
                  "@type":"canonical",
                  "$":"www.youtube.com/"
                },
                "NavigableUrl":{
                  "$":"http://www.youtube.com/"
                },
                "Title":{
                  "$":"YouTube"
                }
              },
              {
                "DataUrl":{
                  "@type":"canonical",
                  "$":"www.yahoo.com/"
                },
                "NavigableUrl":{
                  "$":"http://www.yahoo.com/"
                },
                "Title":{
                  "$":"Yahoo!"
                }
              },
              {
                "DataUrl":{
                  "@type":"canonical",
                  "$":"www.writingwhitepapers.com/"
                },
                "NavigableUrl":{
                  "$":"http://www.writingwhitepapers.com/"
                },
                "Title":{
                  "$":"Writing White Papers"
                }
              },
              {
                "DataUrl":{
                  "@type":"canonical",
                  "$":"www.writelit.com/"
                },
                "NavigableUrl":{
                  "$":"http://www.writelit.com/"
                },
                "Title":{
                  "$":"WriteLit.com"
                }
              },
              {
                "DataUrl":{
                  "@type":"canonical",
                  "$":"www.woopra.com/"
                },
                "NavigableUrl":{
                  "$":"http://www.woopra.com/"
                },
                "Title":{
                  "$":"Woopra"
                }
              },
              {
                "DataUrl":{
                  "@type":"canonical",
                  "$":"www.wikipedia.org/"
                },
                "NavigableUrl":{
                  "$":"http://www.wikipedia.org/"
                },
                "Title":{
                  "$":"Wikipedia"
                }
              },
              {
                "DataUrl":{
                  "@type":"canonical",
                  "$":"www.wholinks2me.com/"
                },
                "NavigableUrl":{
                  "$":"http://www.wholinks2me.com/"
                },
                "Title":{
                  "$":"wholinks2me.com"
                }
              },
              {
                "DataUrl":{
                  "@type":"canonical",
                  "$":"www.whatsnextblog.com/"
                },
                "NavigableUrl":{
                  "$":"http://www.whatsnextblog.com/"
                },
                "Title":{
                  "$":"B.L. Ochman's Whatsnextblog"
                }
              },
              {
                "DataUrl":{
                  "@type":"canonical",
                  "$":"www.webinknow.com/"
                },
                "NavigableUrl":{
                  "$":"http://www.webinknow.com/"
                },
                "Title":{
                  "$":"Web Ink Now"
                }
              }
            ]
          }
        },
        "TrafficData":{
          "DataUrl":{
            "@type":"canonical",
            "$":"feedblitz.com"
          },
          "Rank":{
            "$":53960
          },
          "RankByCountry":{
            "Country":[
              {
                "@Code":"IN",
                "Rank":{
                  "$":29491
                },
                "Contribution":{
                  "PageViews":{
                    "$":"10.0%"
                  },
                  "Users":{
                    "$":"8.6%"
                  }
                }
              },
              {
                "@Code":"SG",
                "Rank":{
                  "$":44897
                },
                "Contribution":{
                  "PageViews":{
                    "$":"0.3%"
                  },
                  "Users":{
                    "$":"0.6%"
                  }
                }
              },
              {
                "@Code":"PH",
                "Rank":{
                  "$":27847
                },
                "Contribution":{
                  "PageViews":{
                    "$":"0.5%"
                  },
                  "Users":{
                    "$":"0.7%"
                  }
                }
              },
              {
                "@Code":"ES",
                "Rank":{
                  "$":65628
                },
                "Contribution":{
                  "PageViews":{
                    "$":"1.5%"
                  },
                  "Users":{
                    "$":"1.6%"
                  }
                }
              },
              {
                "@Code":"US",
                "Rank":{
                  "$":19476
                },
                "Contribution":{
                  "PageViews":{
                    "$":"54.1%"
                  },
                  "Users":{
                    "$":"51.4%"
                  }
                }
              },
              {
                "@Code":"IR",
                "Rank":{
                  "$":95352
                },
                "Contribution":{
                  "PageViews":{
                    "$":"0.4%"
                  },
                  "Users":{
                    "$":"0.9%"
                  }
                }
              },
              {
                "@Code":"AU",
                "Rank":{
                  "$":73713
                },
                "Contribution":{
                  "PageViews":{
                    "$":"0.6%"
                  },
                  "Users":{
                    "$":"0.8%"
                  }
                }
              },
              {
                "@Code":"NG",
                "Rank":{
                  "$":9685
                },
                "Contribution":{
                  "PageViews":{
                    "$":"2.0%"
                  },
                  "Users":{
                    "$":"3.5%"
                  }
                }
              },
              {
                "@Code":"GB",
                "Rank":{
                  "$":29143
                },
                "Contribution":{
                  "PageViews":{
                    "$":"9.3%"
                  },
                  "Users":{
                    "$":"5.2%"
                  }
                }
              },
              {
                "@Code":"PT",
                "Rank":{
                  "$":20442
                },
                "Contribution":{
                  "PageViews":{
                    "$":"2.6%"
                  },
                  "Users":{
                    "$":"0.7%"
                  }
                }
              },
              {
                "@Code":"BR",
                "Rank":{
                  "$":75169
                },
                "Contribution":{
                  "PageViews":{
                    "$":"0.9%"
                  },
                  "Users":{
                    "$":"1.3%"
                  }
                }
              },
              {
                "@Code":"IT",
                "Rank":{
                  "$":74487
                },
                "Contribution":{
                  "PageViews":{
                    "$":"1.2%"
                  },
                  "Users":{
                    "$":"1.6%"
                  }
                }
              },
              {
                "@Code":"ZA",
                "Rank":{
                  "$":48114
                },
                "Contribution":{
                  "PageViews":{
                    "$":"0.4%"
                  },
                  "Users":{
                    "$":"0.7%"
                  }
                }
              },
              {
                "@Code":"CA",
                "Rank":{
                  "$":39291
                },
                "Contribution":{
                  "PageViews":{
                    "$":"1.9%"
                  },
                  "Users":{
                    "$":"2.5%"
                  }
                }
              },
              {
                "@Code":"JP",
                "Rank":{
                  "$":82327
                },
                "Contribution":{
                  "PageViews":{
                    "$":"3.4%"
                  },
                  "Users":{
                    "$":"3.6%"
                  }
                }
              },
              {
                "@Code":"O",
                "Rank":{

                },
                "Contribution":{
                  "PageViews":{
                    "$":"9.6%"
                  },
                  "Users":{
                    "$":"14.2%"
                  }
                }
              },
              {
                "@Code":"DE",
                "Rank":{
                  "$":100216
                },
                "Contribution":{
                  "PageViews":{
                    "$":"1.3%"
                  },
                  "Users":{
                    "$":"2.3%"
                  }
                }
              }
            ]
          },
          "UsageStatistics":{
            "UsageStatistic":[
              {
                "TimeRange":{
                  "Months":{
                    "$":3
                  }
                },
                "Rank":{
                  "Value":{
                    "$":53960
                  },
                  "Delta":{
                    "$":14971
                  }
                },
                "Reach":{
                  "Rank":{
                    "Value":{
                      "$":54136
                    },
                    "Delta":{
                      "$":13036
                    }
                  },
                  "PerMillion":{
                    "Value":{
                      "$":16.4
                    },
                    "Delta":{
                      "$":"-32.40%"
                    }
                  }
                },
                "PageViews":{
                  "PerMillion":{
                    "Value":{
                      "$":1.03
                    },
                    "Delta":{
                      "$":"-45.56%"
                    }
                  },
                  "Rank":{
                    "Value":{
                      "$":62058
                    },
                    "Delta":{
                      "$":21622
                    }
                  },
                  "PerUser":{
                    "Value":{
                      "$":2.63
                    },
                    "Delta":{
                      "$":"-19.57%"
                    }
                  }
                }
              },
              {
                "TimeRange":{
                  "Months":{
                    "$":1
                  }
                },
                "Rank":{
                  "Value":{
                    "$":60882
                  },
                  "Delta":{
                    "$":5364
                  }
                },
                "Reach":{
                  "Rank":{
                    "Value":{
                      "$":58042
                    },
                    "Delta":{
                      "$":1776
                    }
                  },
                  "PerMillion":{
                    "Value":{
                      "$":15
                    },
                    "Delta":{
                      "$":"-4.21%"
                    }
                  }
                },
                "PageViews":{
                  "PerMillion":{
                    "Value":{
                      "$":0.78
                    },
                    "Delta":{
                      "$":"-24.89%"
                    }
                  },
                  "Rank":{
                    "Value":{
                      "$":78115
                    },
                    "Delta":{
                      "$":15550
                    }
                  },
                  "PerUser":{
                    "Value":{
                      "$":2.2
                    },
                    "Delta":{
                      "$":"-21.45%"
                    }
                  }
                }
              },
              {
                "TimeRange":{
                  "Days":{
                    "$":7
                  }
                },
                "Rank":{
                  "Value":{
                    "$":60324
                  },
                  "Delta":{
                    "$":1467
                  }
                },
                "Reach":{
                  "Rank":{
                    "Value":{
                      "$":57463
                    },
                    "Delta":{
                      "$":321
                    }
                  },
                  "PerMillion":{
                    "Value":{
                      "$":16
                    },
                    "Delta":{
                      "$":"-0.14%"
                    }
                  }
                },
                "PageViews":{
                  "PerMillion":{
                    "Value":{
                      "$":0.77
                    },
                    "Delta":{
                      "$":"-8.136%"
                    }
                  },
                  "Rank":{
                    "Value":{
                      "$":79100
                    },
                    "Delta":{
                      "$":5423
                    }
                  },
                  "PerUser":{
                    "Value":{
                      "$":2.1
                    },
                    "Delta":{
                      "$":"-7.860%"
                    }
                  }
                }
              },
              {
                "TimeRange":{
                  "Days":{
                    "$":1
                  }
                },
                "Rank":{
                  "Value":{
                    "$":79384
                  },
                  "Delta":{
                    "$":30187
                  }
                },
                "Reach":{
                  "Rank":{
                    "Value":{
                      "$":75726
                    },
                    "Delta":{
                      "$":29211
                    }
                  },
                  "PerMillion":{
                    "Value":{
                      "$":11
                    },
                    "Delta":{
                      "$":"-45.06%"
                    }
                  }
                },
                "PageViews":{
                  "PerMillion":{
                    "Value":{
                      "$":0.54
                    },
                    "Delta":{
                      "$":"-48.103%"
                    }
                  },
                  "Rank":{
                    "Value":{
                      "$":110137
                    },
                    "Delta":{
                      "$":41327
                    }
                  },
                  "PerUser":{
                    "Value":{
                      "$":2.0
                    },
                    "Delta":{
                      "$":"-5.742%"
                    }
                  }
                }
              }
            ]
          },
          "ContributingSubdomains":{
            "ContributingSubdomain":[
              {
                "DataUrl":{
                  "$":"app.feedblitz.com"
                },
                "TimeRange":{
                  "Months":{
                    "$":1
                  }
                },
                "Reach":{
                  "Percentage":{
                    "$":"70.97%"
                  }
                },
                "PageViews":{
                  "Percentage":{
                    "$":"64.08%"
                  },
                  "PerUser":{
                    "$":2.0
                  }
                }
              },
              {
                "DataUrl":{
                  "$":"feedblitz.com"
                },
                "TimeRange":{
                  "Months":{
                    "$":1
                  }
                },
                "Reach":{
                  "Percentage":{
                    "$":"36.53%"
                  }
                },
                "PageViews":{
                  "Percentage":{
                    "$":"21.03%"
                  },
                  "PerUser":{
                    "$":1.3
                  }
                }
              },
              {
                "DataUrl":{
                  "$":"forms.feedblitz.com"
                },
                "TimeRange":{
                  "Months":{
                    "$":1
                  }
                },
                "Reach":{
                  "Percentage":{
                    "$":"10.27%"
                  }
                },
                "PageViews":{
                  "Percentage":{
                    "$":"4.77%"
                  },
                  "PerUser":{
                    "$":1.0
                  }
                }
              },
              {
                "DataUrl":{
                  "$":"feeds.feedblitz.com"
                },
                "TimeRange":{
                  "Months":{
                    "$":1
                  }
                },
                "Reach":{
                  "Percentage":{
                    "$":"8.97%"
                  }
                },
                "PageViews":{
                  "Percentage":{
                    "$":"4.58%"
                  },
                  "PerUser":{
                    "$":1.1
                  }
                }
              },
              {
                "DataUrl":{
                  "$":"archive.feedblitz.com"
                },
                "TimeRange":{
                  "Months":{
                    "$":1
                  }
                },
                "Reach":{
                  "Percentage":{
                    "$":"5.22%"
                  }
                },
                "PageViews":{
                  "Percentage":{
                    "$":"3.73%"
                  },
                  "PerUser":{
                    "$":1.6
                  }
                }
              },
              {
                "DataUrl":{
                  "$":"support.feedblitz.com"
                },
                "TimeRange":{
                  "Months":{
                    "$":1
                  }
                },
                "Reach":{
                  "Percentage":{
                    "$":"0.84%"
                  }
                },
                "PageViews":{
                  "Percentage":{
                    "$":"0.96%"
                  },
                  "PerUser":{
                    "$":3
                  }
                }
              },
              {
                "DataUrl":{
                  "$":"p.feedblitz.com"
                },
                "TimeRange":{
                  "Months":{
                    "$":1
                  }
                },
                "Reach":{
                  "Percentage":{
                    "$":"0.82%"
                  }
                },
                "PageViews":{
                  "Percentage":{
                    "$":"0.44%"
                  },
                  "PerUser":{
                    "$":1
                  }
                }
              },
              {
                "DataUrl":{
                  "$":"OTHER"
                },
                "TimeRange":{
                  "Months":{
                    "$":1
                  }
                },
                "Reach":{
                  "Percentage":{
                    "$":0
                  }
                },
                "PageViews":{
                  "Percentage":{
                    "$":"0.43%"
                  },
                  "PerUser":{
                    "$":0
                  }
                }
              }
            ]
          }
        }
      }
    }
  }

*******
Methods
*******
.. currentmodule:: awis_action

.. automethod:: UrlInfo.__init__
